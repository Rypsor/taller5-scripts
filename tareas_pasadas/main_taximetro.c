/********************************************************************************
 *  TAXIMETRO CONTROLADO POR ENCODER
 * author: Samuel Botero Rivera
 ********************************************************************************
 *
 ...
 *
 * 1.  **LED de Estado:** Un LED en PH1 parpadea a 1 Hz  para indicar
 * que el sistema está funcionando. Esta tarea es controlada por el timer 4 (TIM4).
 
 * 2.  **Display 7 Segmentos:** Muestra un número de 0 a 4095. El refresco del
 * display se realiza por multiplexación (POV) a 50 Hz ( 5ms por digito, 20 ms todo el display) para evitar
 * parpadeos, controlado por la interrupción del timer 3 (TIM3).
 *
 * 3.  **Encoder Rotatorio:**
 ...
 * automáticamente entre 0 y 4095.
...
#define LED_PH1_PORT          GPIOH     // LED de estado
#define LED_PH1_PIN           GPIO_PIN_1  // LED de estado

// --- Display (Dígitos) ---
#define GPIO_UNIDADES         GPIOB       // puerto dígito unidades
#define GPIO_PIN_UNIDADES     GPIO_PIN_7  // pin dígito unidades

#define GPIO_DECENAS          GPIOC       // puerto dígito decenas
#define GPIO_PIN_DECENAS      GPIO_PIN_6  // pin dígito decenas

#define GPIO_CENTENAS         GPIOC       // puerto dígito centenas
#define GPIO_PIN_CENTENAS     GPIO_PIN_5  // pin dígito centenas

#define GPIO_UNIDADES_MIL     GPIOC       // puerto dígito unidades de mil
#define GPIO_PIN_UNIDADES_MIL GPIO_PIN_13 // pin dígito unidades de mil

// --- Display (Segmentos) --------------------------------------------------------------

#define GPIO_A_PORT          GPIOB          // puerto segmento A
#define GPIO_A_PIN           GPIO_PIN_12    // pin segmento A

#define GPIO_B_PORT          GPIOA          // puerto segmento B
#define GPIO_B_PIN           GPIO_PIN_12    // pin segmento B

#define GPIO_C_PORT          GPIOC          // puerto segmento C
#define GPIO_C_PIN           GPIO_PIN_11    // pin segmento C

#define GPIO_D_PORT          GPIOC          // puerto segmento D
#define GPIO_D_PIN           GPIO_PIN_12    // pin segmento D

#define GPIO_E_PORT          GPIOC          // puerto segmento E
#define GPIO_E_PIN           GPIO_PIN_10    // pin segmento E

#define GPIO_F_PORT          GPIOA          // puerto segmento F
#define GPIO_F_PIN           GPIO_PIN_11    // pin segmento F

#define GPIO_G_PORT          GPIOD          // puerto segmento G
#define GPIO_G_PIN           GPIO_PIN_2     // pin segmento G

// --- Encoder --------------------------------------------------------------

#define ENCODER_CLK_PORT     GPIOB          // puerto pin CLK del encoder
#define ENCODER_PIN_CLK      GPIO_PIN_2     // pin CLK del encoder

#define ENCODER_DATA_PORT    GPIOB          // puerto pin DATA del encoder
#define ENCODER_PIN_DATA     GPIO_PIN_1     // pin DATA del encoder

#define ENCODER_SW_PORT      GPIOB          // puerto pin SW del encoder
#define ENCODER_PIN_SW       GPIO_PIN_15    // pin SW del encoder
...
volatile uint8_t ENCODER_FLAG = 0;       // Flag para el encoder
volatile uint8_t BUTTON_FLAG = 0;        // Flag para el botón

volatile uint8_t  g_system_state = STATE_NORMAL;       // Estado del sistema
volatile int8_t   g_countdown_value = 10;              // Valor de la cuenta regresiva
volatile uint16_t g_countdown_ms_counter = 0;          // Contador de ms para la cuenta regresiva

uint16_t g_encoder_value = 0;                           // Valor del encoder
uint8_t Unidad_mil, Centenas, Decenas, Unidades;       // Dígitos para el display


// ==== Prototipos de funciones ====
void init_display_pins(void);                   // Inicializa los pines del display
void init_encoder_pins(void);                   // Inicializa los pines del encoder
void init_display_timer(void);                  // Inicializa el timer para el display
void init_blinky_timer(void);                   // Inicializa el timer para el LED blinky
void lightNumber(uint8_t number);               // Enciende un número en el display
void update_display_digits(uint16_t value);     // Actualiza los dígitos del display    
...
int main (void){                               // Función principal
    ...
    init_display_timer();                       // inicializar el timer para el display
    init_blinky_timer();                        // inicializar el timer para el LED blinky

    // Iniciar los timers en modo interrupción
    HAL_TIM_Base_Start_IT(&htim3);              // Iniciar el timer 3 en modo interrupción
    HAL_TIM_Base_Start_IT(&htim4);              // Iniciar el timer 4 en modo interrupción

    update_display_digits(g_encoder_value);     // actualizar los dígitos del display
    ...
    while (1){                                                          // Bucle principal
        ...
        if (TIMER_DISPLAY_FLAG) {                       // Si el timer de display se ha disparado
            TIMER_DISPLAY_FLAG = 0;                     // Reiniciar el flag del timer
            ...
            HAL_GPIO_WritePin(GPIO_UNIDADES_MIL, GPIO_PIN_UNIDADES_MIL, GPIO_PIN_SET);  // Apagar digitos de unidades de mil
            HAL_GPIO_WritePin(GPIO_CENTENAS, GPIO_PIN_CENTENAS, GPIO_PIN_SET);          // Apagar digitos de centenas
            HAL_GPIO_WritePin(GPIO_DECENAS, GPIO_PIN_DECENAS, GPIO_PIN_SET);            // Apagar digitos de decenas
            HAL_GPIO_WritePin(GPIO_UNIDADES, GPIO_PIN_UNIDADES, GPIO_PIN_SET);          // Apagar digitos de unidades

            static uint8_t current_digit = 0;

            // switch case para encender cada dígito por turno
            switch(current_digit){
                case 0: lightNumber(Unidades); HAL_GPIO_WritePin(GPIO_UNIDADES, GPIO_PIN_UNIDADES, GPIO_PIN_RESET); break;
                case 1: lightNumber(Decenas); HAL_GPIO_WritePin(GPIO_DECENAS, GPIO_PIN_DECENAS, GPIO_PIN_RESET); break;
                case 2: lightNumber(Centenas); HAL_GPIO_WritePin(GPIO_CENTENAS, GPIO_PIN_CENTENAS, GPIO_PIN_RESET); break;
                case 3: lightNumber(Unidad_mil); HAL_GPIO_WritePin(GPIO_UNIDADES_MIL, GPIO_PIN_UNIDADES_MIL, GPIO_PIN_RESET); break;
            }
        ...
        if (ENCODER_FLAG && g_system_state == STATE_NORMAL) {  
            ENCODER_FLAG = 0;
            if (HAL_GPIO_ReadPin(ENCODER_DATA_PORT, ENCODER_PIN_DATA) == GPIO_PIN_RESET) { // CCW
                if (g_encoder_value == 0) g_encoder_value = 4095;
                else g_encoder_value--;
            } else { // CW
                if (g_encoder_value == 4095) g_encoder_value = 0;
                else g_encoder_value++;
            }
            update_display_digits(g_encoder_value);
        }
...
// calculo de los digitos para el display
...
// Inicializa los pines del display
void init_display_pins(void) {
    ...
    // LED
    GPIO_InitStruct.Pin = LED_PH1_PIN; HAL_GPIO_Init(LED_PH1_PORT, &GPIO_InitStruct);
...
void init_display_timer(void) {
    ...
    //  16,000,000 / 1600 = 10,000 Hz. 10,000 Hz / 50 = 200 Hz -> 5ms
    htim3.Instance = TIM3;
...
void init_blinky_timer(void) {
    ...
    // 16,000,000 / 16000 = 1,000 Hz. 1,000 Hz / 500 = 2 Hz -> 500ms
    htim4.Instance = TIM4;
...
// Se llama cuando se dispara cualquier interrupción de pines del encoder
