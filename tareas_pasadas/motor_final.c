 ...
 *
 * Descripción general:
 * Este programa controla un motor DC mediante una placa STM32, usando PWM para velocidad y GPIO para dirección/freno. El usuario puede interactuar con el motor a través de comandos enviados por comunicación serial (UART), donde cada letra corresponde a una función específica:
 ...
 *
 * El código utiliza interrupciones para procesar los comandos seriales y para medir la frecuencia del motor mediante Input Capture. El control del motor se realiza actualizando el PWM y la dirección según el estado y el botón recibido.
 *
...
/* -----------------------------------------------------------------------------
 * Pin mapping / conexiones (símbolos usados en el código)
 *  - Motor IN1  -> TIM5 CH1 (salida PWM, corresponde a htim5 channel 1)
 *  - Motor IN2  -> TIM5 CH2 (salida PWM, corresponde a htim5 channel 2)
 *  - Sensor/tach (entrada) -> TIM3 CH1 (Input Capture, corresponde a htim3 channel 1)
 *  - LED Blinky -> PIN_BLINKY_Pin  (definido en main.h)
 *  - User LED   -> LD2_Pin         (definido en main.h) 
 * --------------------------------------------------------------------------- */
...
// htim2: Timer usado para el LED Blinky (Periodo configurado en MX_TIM2_Init)
// htim3: Timer usado para Input Capture (medición de frecuencia del motor)
// htim5: Timer usado para PWM del motor (canales CH1 y CH2)
// huart2: Puerto UART usado para la comunicación serie con el PC
...
// Rango de 'velocidad' está pensado en porcentaje (0..100). Duty cycle real se calcula
// en actualizarMotor() como (velocidad * 999) / 100 para PWM con periodo 0..999.
...
int main(void)
{
	...
	MX_TIM2_Init();
	/* USER CODE BEGIN 2 */
	// Inicia PWM en TIM5 canales 1 y 2 (control del motor)
	HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_1);
	HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_2);
	// Inicia TIM2 en modo base con interrupción para el Blinky
	HAL_TIM_Base_Start_IT(&htim2);
	// Inicia Input Capture en TIM3 CH1 con interrupción para medir la frecuencia del motor
...
static void MX_TIM2_Init(void)
{
	...
	htim2.Instance = TIM2;
	// Prescaler y Periodo para TIM2: con PSC=16000-1 y APB clock en 16MHz -> 1 kHz tick
...
static void MX_TIM3_Init(void)
{
	...
	// Filtro digital del Input Capture para reducir rebotes/ruido (0..15)
...
static void MX_TIM5_Init(void)
{
	...
	htim5.Instance = TIM5;
	// TIM5 prescaler y period definen la resolución PWM. PSC=16-1 reduce frecuencia del timer.
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM3)
	{
		...
		else
		{
			...
			{
				// Timer desbordado/* USER CODE BEGIN Header */
/**
 ******************************************************************************
 * @file           : main.c
 * @brief          : Main program body
...
uint8_t Bandera_USART_RX = 0;
uint8_t botonPresionado = 0;
uint8_t letraRecibida = 0;
uint8_t g_dato_rx;

/* --- Variables para Medición de Frecuencia (TIM3) --- */
...
uint32_t leerFrecuenciaMotor(void)
{
    // Comprueba si ha pasado mucho tiempo (500ms) desde la última captura
    if ( (HAL_GetTick() - g_last_capture_time) > 500 )
    {
        // Ha pasado más de medio segundo. Asumimos 0 Hz.
        g_frecuencia_medida = 0;
        g_is_first_capture = 1; // Resetea la lógica de captura
    }
    
    // Devuelve el último valor (ya sea el calculado o 0 por timeout)
    return g_frecuencia_medida;
}

// Función para enviar la frecuencia del motor - CORREGIDA
void enviarFrecuenciaMotor(void)
{
    // Llama a la nueva función no bloqueante
    uint32_t frecuencia = leerFrecuenciaMotor(); 
    char buffer[50];
    
    int longitud = sprintf(buffer, "Frecuencia del motor: %lu Hz\r\n", frecuencia);

    // Usa la función HAL para transmitir (más seguro)
    HAL_UART_Transmit(&huart2, (uint8_t*)buffer, longitud, HAL_MAX_DELAY);
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    // Asegurarnos de que es la interrupción del TIM3
    if (htim->Instance == TIM3)
    {
        // Registra el tiempo actual para el timeout
        g_last_capture_time = HAL_GetTick(); 

        if(g_is_first_capture)
        {
            // 1. Es la primera captura, solo guardamos el valor
            g_capture_val_1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
            g_is_first_capture = 0;
        }
        else
        {
            // 2. Es la segunda captura (o posterior)
            g_capture_val_2 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);

            // 3. Calculamos la diferencia de "ticks" entre pulsos
            if (g_capture_val_2 > g_capture_val_1)
            {
                g_capture_diff = g_capture_val_2 - g_capture_val_1;
            }
            ...
            {
                // El timer se ha desbordado (ha vuelto a 0)
                // 65535 es el valor de ARR (Period)
            ...
            if(g_capture_diff > 0)
            {
                // Obtenemos la frecuencia del reloj del timer (1MHz o 10kHz)
                ...
                // Frecuencia = Freq_tick / N_ticks
                g_frecuencia_medida = timer_tick_freq / g_capture_diff;
            }

            // 5. La "segunda" captura de hoy es la "primera" de la próxima medición
