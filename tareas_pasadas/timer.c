...
volatile uint8_t g_request_curva_ic_vc = 0;
  ...
  while (1)
  {
	...
	if (g_request_adc_vb)
	{
      ...
      if (g_request_adc_ic)
          {
              ...
              if (vc_pwm_mv > vc_medido_mv) {
                  ic_ua = (vc_pwm_mv - vc_medido_mv) * 1000 / 4000;
              }
      ...
      if (g_request_curva_ic_vc)
      {
		  ...
		  // 1. Leer Vc actual para la cabecera
		  uint16_t adc_vc_ref = leer_canal_adc(ADC_CHANNEL_6);
		  uint32_t vc_ref_mv = (uint32_t)adc_vc_ref * 3300 / 4095;

		  // 2. Imprimir cabeceras
          HAL_UART_Transmit(&huart2, (uint8_t*)"\n--- Curva Ic vs Vb (con Vc fijado a ~", 40, 200);
          sprintf(temp_buf, "%u", (unsigned int)(vc_ref_mv / 1000));
          HAL_UART_Transmit(&huart2, (uint8_t*)temp_buf, strlen(temp_buf), 100);
          HAL_UART_Transmit(&huart2, (uint8_t*)".", 1, 100);
          uint32_t vc_ref_frac = vc_ref_mv % 1000;
          if (vc_ref_frac < 100) HAL_UART_Transmit(&huart2, (uint8_t*)"0", 1, 100);
          if (vc_ref_frac < 10) HAL_UART_Transmit(&huart2, (uint8_t*)"0", 1, 100);
          sprintf(temp_buf, "%u", (unsigned int)vc_ref_frac);
          HAL_UART_Transmit(&huart2, (uint8_t*)temp_buf, strlen(temp_buf), 100);
          HAL_UART_Transmit(&huart2, (uint8_t*)"V) ---\n", 8, 200);
		  ...
		  // 3. Barrer Vb y medir Ic
		  for (uint16_t vb_porc = 0; vb_porc <= 100; vb_porc += 5)
		  {
			  // a. Set Vb
			  uint16_t vb_pwm = ((uint32_t)vb_porc * 1023) / 100;
			  __HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_1, vb_pwm);
			  ...
			  // c. Medir Vc real (para el cálculo de Ic)
			  uint16_t adc_vc = leer_canal_adc(ADC_CHANNEL_6);
			  ...
			  uint32_t ic_ua = 0;
			  if (vc_supply_mv > vc_medido_mv) {
				  ic_ua = (vc_supply_mv - vc_medido_mv) * 1000 / 4000;
			  }

			  // e. Imprimir Vb medido (V)
              sprintf(temp_buf, "%u", (unsigned int)(vb_medido_mv / 1000));
              HAL_UART_Transmit(&huart2, (uint8_t*)temp_buf, strlen(temp_buf), 100);
              HAL_UART_Transmit(&huart2, (uint8_t*)".", 1, 100);
              uint32_t vb_frac = vb_medido_mv % 1000;
		  ...
		  if (g_request_status_x)
		  {
			  ...
			  if (vc_supply_mv > vc_mv) {
				  ic_ua = (vc_supply_mv - vc_mv) * 1000 / 4000;
			  }
