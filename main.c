...
/* USER CODE BEGIN PV */
uint8_t rx_byte;
const char msg_respuesta[] = "\r\n[DMA]: Tecla 'a' recibida correctamente.\r\n";
// Buffer para almacenar los valores RAW del ADC (0 a 4095)
// indice 0 = Eje X (PA0), indice 1 = Eje Y (PA1)
uint16_t joystick_data[2];

// Buffer para mensaje UART
char msg_buffer[50];
/* USER CODE END PV */
...
{
  // Verificamos que sea el UART2
  if (huart->Instance == USART2)
  {
    // Chequeamos el dato
    if (rx_byte == 'a')
    {
       // Enviamos respuesta (también por DMA para no bloquear)
       HAL_UART_Transmit_DMA(&huart2, (uint8_t*)msg_respuesta, strlen(msg_respuesta));
    }

    // IMPORTANTE: Volver a activar la escucha para la siguiente tecla
    HAL_UART_Receive_DMA(&huart2, &rx_byte, 1);
  }
}
/* USER CODE END 4 */
